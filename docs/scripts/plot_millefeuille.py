...
def initialize(name, i):
    ...
    cohesion = Function(sDG0)

    return visc, visc_log, mesh, sDG0 #Temp, v, visc, mesh, visc_log, cohesion, sDG0

def load(i, visc, name):
    Temp_dataset = "/temperature/vector_%d"% (i)
    v_dataset = "/velocity/vector_%d"% (i)
    visc_dataset = "/viscosity/vector_%d"% (i)
    cohesion_dataset = "/cohesion/vector_%d"% (i)
    ...
    # files_hdf5_in['data'].read(cohesion, cohesion_dataset)
...
def load_comp_tracers(ax, name, fig, font_size, i):
    ...
    for line in lines:
        ...
        ocean = float(sline[7])
        surface = float(sline[8])
        origin = float(sline[3])
            
        if (ocean == 1):
            xx_ocean.append(float(sline[0]))
            yy_ocean.append(float(sline[1]))

        if (surface == 1 and origin == 0):
            xx_surface.append(float(sline[0]))
            yy_surface.append(float(sline[1]))


    ax.scatter(xx_ocean, yy_ocean, marker=".", c = "royalblue", alpha = 0.3, s = 10.25)
    ax.scatter(xx_surface, yy_surface, marker=".", c= 'lightsalmon',alpha = 0.3, s = 10.25)
