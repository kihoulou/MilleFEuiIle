...
if (skip_plotting == False):
    ...
    for ii in steps:
        ...
        if (ii[0] > i_start):
            ...
            visc, visc_log, mesh, sDG0 = initialize(name, hdf5_step)
            visc = load(hdf5_step, visc, name)
            ...
            if (show_time == True):
                ax.text(0, 15e3, r"$t$ = "+str("{:.1f}".format(ii[2]/1e3))+" Myr", fontsize=font_size)

            # --- Plot temperature field ---
            print("\nPlotting snapshot", hdf5_step, "/", int(steps[len(steps) - 1][0]))
            plot_scalar(ax, fig, visc_log, mesh, labels, x_range, z_range, font_size)

            # --- Plot tracers ---
            if (plot_tracers == True):
                load_tracers(ax, name, ii[0])

            if (plot_melt_tracers == True):
                load_melt_tracers(ax, name, labels, fig, font_size, ii[0])

            load_comp_tracers(ax, name, fig, font_size, ii[0])
            ...
            if (plot_streamlines == True):
                ...
                plt.savefig("data_" + name + "/img/tracers_" + str(hdf5_step).zfill(3) + ".png", dpi = 150, transparent=False, pad_inches=0.3, bbox_inches='tight')
...
if (plot_streamlines == True):
    ...
    # subprocess.run(["convert -delay 5 data_" + name + "/img/temp_*.png data_" + name + "/anim/tectonics.gif"], shell=True) 
    subprocess.run(["ffmpeg -framerate 24 -i data_" + name + "/img/temp_%03d.png -c:v libx264 -r 30 name.mp4"], shell=True) 
