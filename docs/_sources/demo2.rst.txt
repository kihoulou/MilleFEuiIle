...
Here you can download the :download:`bash script <code/demo2/run_MilleFEuiIle_demo2.sh>`\, the :download:`parameter file <code/demo2/m_parameters_demo2.py>`\ and the :download:`matplotlib script <code/demo2/plot_demo2.py>`\ for the heat flux evolution.
...
We choose to explore viscosities :math:`10^{13},` :math:`10^{13}`, :math:`10^{15}` and :math:`10^{16}` Pa s. As it is easy to
pass an integer from bash script to python, we run the loop over the exponents ``13 14 15 16``:

.. code-block:: shell
    :emphasize-lines: 2, 5, 11, 17, 25
...
First, we incorporate this parameter to the directory name, so that every
simulation has its own directory:
...
The plotting script reads the statistics files from all four simulations.
We provide path to these files, and define the labels that will appear in the legend
and the color and style for each of the lines.

.. code-block:: python
    :emphasize-lines: 2,3,4,5,6,9,10,11,12,15,18
