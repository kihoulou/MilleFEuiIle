...
class MeshModule:
    def __init__(self):
        ...
        if (loading_mesh == True and iSALE_input == False):
            self.Load_Mesh()
            # How to define boundaries???

        # elif (iSALE_input == True):
        #     self.Load_Mesh_iSALE()
        #     # How to define boundaries???
    ...
    def Load_Mesh_iSALE(self):
        self.mesh = Mesh()
        self.mesh_file = HDF5File(comm, "iSALE_IC/data_" + self.name + "/HDF5/mesh.h5", "r")
        self.mesh_file.read(self.mesh, "/mesh", True)
        self.mesh_file.close()
