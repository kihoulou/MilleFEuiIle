...
class Equations:
    def __init__(self, MeshClass, ElemClass, TracersClass, MeltingClass, FilesClass, t):
        ...
        self.Temp_iSALE   = ElemClass.Temp_iSALE
        self.density_iSALE   = ElemClass.density_iSALE
        ...
        self.h_top_iSALE    = ElemClass.h_top_iSALE
        ...
        self.grain_size.assign(project(1e-3, self.sDG0))
    ...
    def equation_heat(self):
        if (nonlinear_heat_equation == True):
            ...
            # if (shear_heating == True):
            #         eq_energy += -inner(self.stress_dev_tensor, self.strain_rate_tensor)*self.Temp_*dx
    ...
    def solve_heat_equation(self):
        # if (shear_heating == True):
        #     self.heating_shear.assign(project(inner(self.stress_dev_tensor, self.strain_rate_tensor), self.sDG0))
    ...
    def solve_Stokes_problem(self, step, FilesClass):
        ...
        while (Picard_iter < Picard_iter_max and v_error > Picard_iter_error):
            ...
            if (self.nonlinear_rheology == True):
                ...
                if ((elasticity == True and viscosity_type == "GK_2001") or grain_size_evolution == True):
                    get_new_stress_iter(self.mesh, self.Temp, self.xm, self.stress_dev_inv,\
                                        ...
                                        self.shear_modulus, self.grain_size, step, Picard_iter, self.dt)

                Picard_iter += 1
                
                if (grain_size_evolution == True):
                    mech = "GBS"
                    strain_rate_invariant_GBS =  eps_i_mesh(mech, self.Temp, self.grain_size, self.stress_dev_inv)

                    mech = "disl"
                    strain_rate_invariant_disl =  eps_i_mesh(mech, self.Temp, self.grain_size, self.stress_dev_inv)

                    self.grain_size.assign(project(steady_grain_size(self.Temp, self.stress_dev_inv, strain_rate_invariant_GBS, strain_rate_invariant_disl, self.strain_rate_inv), self.sDG0))
    ...
    def update_viscosity(self, step, Picard_iter):
        ...
        self.visc.assign(project(eta_eff(self.p_k, self.Temp, self.strain_rate_inv,\
                                        ...
                                        step, Picard_iter, self.stress_dev_inv_k, self.shear_modulus, self.dt, self.sr_min, self.grain_size), self.sDG0))
