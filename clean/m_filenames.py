...
class SaveFiles:
    def __init__(self, MeshClass, ElemClass):
        ...
        Path("iSALE_IC/data_" + self.name + "/").mkdir(parents = True, exist_ok = True)
        Path("iSALE_IC/data_" + self.name + "/paraview").mkdir(parents = True, exist_ok = True)
        ...
        self.grain_size = ElemClass.grain_size
        ...
        Paraview_Output_Ini_iSALE = ["temperature", "density", "topography_top"]
        for i in range(len(Paraview_Output_Ini_iSALE)):
            self.Paraview_Dict_Ini_iSALE[Paraview_Output_Ini_iSALE[i]] = XDMFFile(comm,"iSALE_IC/data_" + self.name + "/paraview/" + Paraview_Output_Ini_iSALE[i] + ".xdmf")
            self.Paraview_Dict_Ini_iSALE[Paraview_Output_Ini_iSALE[i]].parameters["flush_output"] = True
            self.Paraview_Dict_Ini_iSALE[Paraview_Output_Ini_iSALE[i]].parameters["rewrite_function_mesh"] = True

            if (Paraview_Output_Ini_iSALE[i] == "temperature"):
                self.Function_Dict[Paraview_Output_Ini_iSALE[i]] = self.Temp
            
            if (Paraview_Output_Ini_iSALE[i] == "density"):
                self.Function_Dict[Paraview_Output_Ini_iSALE[i]] = self.density

            if (Paraview_Output_Ini_iSALE[i] == "topography_top"):
                self.Function_Dict[Paraview_Output_Ini_iSALE[i]] = self.h_top
        ...
        for i in range(len(Paraview_Output)):
            ...
            if (Paraview_Output[i] == "shear_heating"):
                self.Function_Dict[Paraview_Output[i]] = self.heating_shear

            if (Paraview_Output[i] == "grain_size"):
                self.Function_Dict[Paraview_Output[i]] = self.grain_size
    ...
    def Save_Paraview_Ini_iSALE(self):
        Paraview_Output_Ini_iSALE = ["temperature", "density", "topography_top"]
        
        for i in range(len(Paraview_Output_Ini_iSALE)):
            self.Function_Dict[Paraview_Output_Ini_iSALE[i]].rename(Paraview_Output_Ini[i], "")
            self.Paraview_Dict_Ini_iSALE[Paraview_Output_Ini_iSALE[i]].write(self.Function_Dict[Paraview_Output_Ini[i]])
    ...
    def Save_HDF5_iSALE(self):
        self.data_file_iSALE = {}
        self.data_file_iSALE['data'] = HDF5File(comm, "iSALE_IC/data_" + self.name + "/data.h5", "w")

        Paraview_Output_iSALE = ["temperature", "density", "topography_top"]
        for i in range(len(Paraview_Output_iSALE)):
            self.data_file_iSALE['data'].write(self.Function_Dict[Paraview_Output_iSALE[i]], "/" + Paraview_Output_iSALE[i], 0) 
        
        self.data_file_iSALE['data'].flush()

    def Load_HDF5_iSALE(self, Temp, density, h_top):
        files_hdf5_in = {}
        files_hdf5_in['data'] = HDF5File(comm, "iSALE_IC/data_" + name + "/data.h5", "r")
        
        files_hdf5_in['data'].read(Temp, "/temperature/vector_0")
        files_hdf5_in['data'].read(density, "/density/vector_0")
        files_hdf5_in['data'].read(h_top, "/topography_top/vector_0")
