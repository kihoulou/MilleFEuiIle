...
def get_new_stress(mesh, Temp, xm, stress_dev_inv, stress_dev_inv_k, strain_rate_invariant, composition, shear_modulus, grain_size, step, Picard_iter, dt):
    ranks = []
    eval_type = "local"
    for j in range(mesh.num_cells()):
        ...
        grain_size_cell = grain_size(Point(centroid.x(), centroid.y()))
        ...
        visc_cell = eta_ductile(temp_cell, eps_II_cell, sigma_II_cell, xm_cell, comp_cell, step, Picard_iter, eval_type, grain_size_cell)
...
def get_new_stress_iter(mesh, Temp, xm, stress_dev_inv, stress_dev_inv_k, strain_rate_inv, composition, shear_modulus,grain_size, step, Picard_iter, dt):
    ranks = []
    eval_type = "local"
    ...
    for j in range(mesh.num_cells()):
        ...
        visc_cell = eta_ductile(temp_cell, eps_II_cell, new_stress, xm_cell, comp_cell, step, Picard_iter, eval_type, grain_size_cell)
        ...
        while (error > stress_iter_error and k < 50):
            ...
            visc_cell = eta_ductile(temp_cell, eps_II_cell, stress_c, xm_cell, comp_cell, step, Picard_iter, eval_type, grain_size_cell)
...
def eta_ductile(Temp, strain_rate, stress, xm, composition, step, Picard_iter, eval_type, grain_size):
    ...
    if (viscosity_type == "GK_2001"):
        if (step == 0 and Picard_iter == 0):
            eta_v =  eta_diff(Temp, grain_size)*exp(-45.0*xm) 

        else:               
            eta_v = 1.0/(1.0/eta_diff(Temp, grain_size)\
                    + 1.0/eta_disl(Temp, strain_rate, stress, grain_size, eval_type)\
                    + 1.0/(eta_GBS(Temp, strain_rate, stress, grain_size, eval_type) + eta_BS(Temp, strain_rate, grain_size, stress))\
                    )*exp(-45.0*xm)
...
def eta_eff(p_k, Temp, strain_rate, stress, xm, composition, plastic_strain, step, Picard_iter, stress_dev_inv_k, shear_modulus, dt, sr_min, grain_size):
    
    eval_type = "mesh"

    # --- Ductile viscosity ---
    eta_v = eta_ductile(Temp, strain_rate, stress, xm, composition, step, Picard_iter, eval_type, grain_size)
...
def eta(Q, A, n, m, Temp, strain_rate_inv, stress_inv, grain_size):
    ...
    if (elasticity == False):
        return 1.0/(3.0**((n+1)/(2.0*n))*2.0**((n-1.0)/n))*A**(-1.0/n)*gs**(m/n)*strain_rate_inv**((1.0-n)/n)*exp(Q/(n*R_gas*Temp))
        
    # --- Stress-based formula ---
    if (elasticity == True):
        return 1.0/(3.0**((n+1)/2.0))*A**(-1.0)*gs**m*stress_inv**(1.0-n)*exp(Q/(R_gas*Temp))

def eta_diff(Temp, grain_size):
    V_m = 1.97e-5           #m^3
    ...
    return 1.0/(2*(3.0/2.0*14)*V_m)*R_gas*Temp*gs**2/(D_v + np.pi*delta*D_b/gs)
    
def eta_disl(Temp, strain_rate_II, stress_II, grain_size, eval_type):
    n = 4.0           # -
    p = 0.0           # -
    T_crit = 258      # K
    ...
    if (eval_type == "mesh"):
        ...
        eta(Q_above, A_above, n, p, Temp, strain_rate_II, stress_II, grain_size),\
        eta(Q_below, A_below, n, p, Temp, strain_rate_II, stress_II, grain_size))

    if (eval_type == "local"):
        if (Temp > T_crit):
            return eta(Q_above, A_above, n, p, Temp, strain_rate_II, stress_II, grain_size)
        else:
            return eta(Q_below, A_below, n, p, Temp, strain_rate_II, stress_II, grain_size)

def eta_GBS(Temp, strain_rate_II, stress_II, grain_size, eval_type):
...
def eta_BS(Temp, strain_rate_II, stress_II, grain_size):
    ...
    return eta(Q, A, n, p, Temp, strain_rate_II, stress_II, grain_size)
