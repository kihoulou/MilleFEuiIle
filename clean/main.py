...
def run_code():
    ...
    if (reload_HDF5 == True):
        ...
        if (solve_energy_problem == True and iSALE_input == False):
    ...
    if (iSALE_input == True):
        # --- Load temperature, density and topography ---
        FilesClass.Load_HDF5_iSALE(EqClass.Temp, EqClass.density, EqClass.h_top)
        EqClass.h_top_k.assign(EqClass.h_top)
        ...
        # --- Add melt tracers ---
        MeltingClass.add_melt_tracers()
        MeltingClass.compute_initial_melt(EqClass.Temp_iSALE, EqClass.density)
