...
class Melting:
    ...
    def compute_melting(self):
        ...
        for j in range(self.mesh.num_cells()):
            for i in range(0, len(self.tracers_in_cells[j])):
                ...
                temp_tracer = self.Temp(Point(xx,yy))
                comp_tracer = self.tracers[tracer_no][8]

                if (temp_tracer >= T_melt):
                    # Ice is melting
                    self.tracers[tracer_no][9][0] += cp(T_melt,  comp_tracer)/Lt*(temp_tracer-T_melt)
                    self.tracers[tracer_no][9][1] = T_melt - temp_tracer

                else:
                    if (self.tracers[tracer_no][9][0] > 0.0):
                        if (abs(cp(T_melt,  comp_tracer)/Lt*(temp_tracer-T_melt)) <= self.tracers[tracer_no][9][0]):
                            # Part of the melt crystallizes
                            self.tracers[tracer_no][9][0] += cp(T_melt,  comp_tracer)/Lt*(temp_tracer-T_melt)
                            self.tracers[tracer_no][9][1] = T_melt - temp_tracer
                        else:
                            # All the melt crystallizes
                            self.tracers[tracer_no][9][1] = self.tracers[tracer_no][9][0]*Lt/cp(T_melt,  comp_tracer) # This one imperatively first !
    ...
    def compute_initial_melt(self, Temp, density):
        ...
        if (rank == 0):
            print("\nComputing melt fraction after impact...")
        ...
        for j in range(self.mesh.num_cells()):
            for i in range(0, len(self.tracers_in_cells[j])):
                ...
                density_tracer = density(Point(xx,yy))
